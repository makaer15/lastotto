...
function addInterest2(interest){
  connection.query('insert into user_table(username,interest) values(\'' + users_name + '\', \'' + interest + '\')', function(error, rows, fields){
    if (error){
      console.log('error:', error);
      throw error;
    }
...
app.post('/webhook/', function(req, res) {
	...
	for(let i = 0; i < messaging_events.length; i++) {
      ...
      if (text.includes("merhaba") || text.includes("Merhaba")){
        getUsername(sender,true);
      }else if (text.includes("arkadaş")){
         getPeopleWithInterests(sender);
				sendText(sender, "Arıyoruz.")
